...
app.get('/api', async (req, res) => {
	if (parseInt(req.query.page)) {
		if (req.query.CHAVES) {
			let raw = await fetch(
				`http://tecnojobs.pt/default.asp?page=${req.query.page}&CHAVES=${req.query.CHAVES}`
			);
			...
			let raw = await fetch(
				`http://tecnojobs.pt/default.asp?page=${req.query.page}`
			);
			let html = await raw.text();
		...
		if (req.query.CHAVES) {
			let raw = await fetch(
				`http://www.tecnojobs.pt/default.asp?chaves=${req.query.CHAVES}`
			);
			let html = await raw.text();
