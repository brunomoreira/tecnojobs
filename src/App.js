  ...
  const getPage = async (pageNum) => {
    ...
    try {
      ...
      if(pageNum === 1 || pageNum === 0) {
        rawRes = await fetch(`http://localhost:4000/api`, { method: 'GET', headers: { 'Content-Type': 'text/html' } })
      } else {
        rawRes = await fetch(`http://localhost:4000/api?page=${pageNum}`, { method: 'GET', headers: { 'Content-Type': 'text/html' } })
      }
